...
int main (int argc, char *argv[]) {
   ...
   bits8 byte = 0;
   // if currentColor is 0, byte is 0
   // if currentColor is 1, byte is 255
   int counter1 = 0;
   int counter2 = 0;
   int counter3 = 0;
   int counter4 = 0;
   int dimensions = SIZE/squareSize;
   int bytesPerSquare = squareSize * 3;
       while (counter1 < dimensions){ // lines
            while (counter2 < squareSize) { // squares
                    while (counter3 < dimensions){ // lines per square
                        while (counter4 < bytesPerSquare){ // bytes per square
