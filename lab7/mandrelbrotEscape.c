 ...
 *  escapeSteps function for the Mandrelbrot project (Task2)
 *  Lab6, Module6
 */
...
int escapeSteps (double x, double y){
    ...
    // While steps is less than MAX_STEPS and distance (sqrt(a^2+b^2)) less
    // than or equal to 2.
    while (steps < MAX_STEPS && distance <= 2){
        i.real = (((z.real)*(z.real)) - ((z.imaginary)*(z.imaginary))+c.real);
        i.imaginary = ((2*(z.real)*(z.imaginary)) + c.imaginary);
        distance = sqrt((i.real)*(i.real)+(i.imaginary)*(i.imaginary));
        z.imaginary = i.imaginary;
        z.real = i.real;
        steps++;
    }
...
void testCases(){
   ...
   printf("    ========================================================");
   printf("\n\n    All test cases passed, as Richard says - You're awesome!\n\n");
   printf("    ========================================================");
