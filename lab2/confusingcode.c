...
// get input
    ...
    numScans += scanf("%lf,%lf", &x0,&y0);
    printf("Triangle Vertex  B (x,y): "); 
    numScans += scanf("%lf,%lf", &x1,&y1);
    printf("Triangle Vertex  C (x,y): "); s
    numScans += scanf("%lf,%lf", &x2,&y2);
    printf("Test Point (x,y): "); 
    numScans += scanf("%lf,%lf", &px,&py);
 
// print error
   if(numScans != 8){
   		printf("You didn't put in the right inputs!\n");
   }
   return 1;
}
...
int test2(double px, double py, double m, double b) {	
	int retValue;
	if( py < m * px + b ) {
		retValue = -1; // point is under line
	} else if ( py == m * px + b ) {
        retValue = 0;
    } else {
        retValue = 1; // over
...
int test1(double px, double py, double m,double b, double lx,double ly) {
	int res1 = test2(px,py,m,b);
	int res2 = test2(lx,ly,m,b);
    return (res1 == res2);
}
